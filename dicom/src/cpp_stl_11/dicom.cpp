...
void dicom_t::t_file_header_t::_read() {
    ...
    if (!(magic() == std::string("\x44\x49\x43\x4D", 4))) {
        throw kaitai::validation_not_equal_error<std::string>(std::string("\x44\x49\x43\x4D", 4), magic(), _io(), std::string("/types/t_file_header/seq/1"));
    }
}
...
void dicom_t::seq_item_t::_read() {
    ...
    if (!(tag_group() == std::string("\xFE\xFF", 2))) {
        throw kaitai::validation_not_equal_error<std::string>(std::string("\xFE\xFF", 2), tag_group(), _io(), std::string("/types/seq_item/seq/0"));
    }
    m_tag_elem = m__io->read_u2le();
    m_value_len = m__io->read_u4le();
    n_value = true;
