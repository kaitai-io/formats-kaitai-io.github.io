...
class Dicom(ReadWriteKaitaiStruct):
    ...
    def _write__seq(self, io=None):
        super(Dicom, self)._write__seq(io)
    ...
    class SeqItem(ReadWriteKaitaiStruct):
        def __init__(self, _io=None, _parent=None, _root=None):
        ...
        def _write__seq(self, io=None):
            super(Dicom.SeqItem, self)._write__seq(io)
            self._io.write_bytes(self.tag_group)
            self._io.write_u2le(self.tag_elem)
            self._io.write_u4le(self.value_len)
            if self.value_len != 4294967295:
                pass
        ...
        def _check(self):
            if len(self.tag_group) != 2:
                raise kaitaistruct.ConsistencyError(u"tag_group", 2, len(self.tag_group))
            if not self.tag_group == b"\xFE\xFF":
                raise kaitaistruct.ValidationNotEqualError(b"\xFE\xFF", self.tag_group, None, u"/types/seq_item/seq/0")
            if self.value_len != 4294967295:
                pass
                if len(self.value) != self.value_len:
                    raise kaitaistruct.ConsistencyError(u"value", self.value_len, len(self.value))
            ...
            if self.value_len == 4294967295:
                ...
                for i in range(len(self.items)):
                    ...
                    _ = self.items[i]
                    if  ((_.tag_group == 65534) and (_.tag_elem == 57357))  != (i == len(self.items) - 1):
                        raise kaitaistruct.ConsistencyError(u"items", i == len(self.items) - 1,  ((_.tag_group == 65534) and (_.tag_elem == 57357)) )


            self._dirty = False


    class TDataElementExplicit(ReadWriteKaitaiStruct):
        """
        .. seealso::
        ...
        def _write__seq(self, io=None):
            super(Dicom.TDataElementExplicit, self)._write__seq(io)
            self._io.write_u2le(self.tag_group)
            self._io.write_u2le(self.tag_elem)
            if (not (self.is_forced_implicit)):
                pass
                self._io.write_bytes((self.vr).encode(u"ASCII"))
            ...
            if _on == False:
                pass
                self._io.write_u2le(self.value_len)
            elif _on == True:
                pass
                self._io.write_u4le(self.value_len)
            if self.value_len != 4294967295:
            ...
            if self.is_transfer_syntax_change_implicit:
                ...
                for i in range(len(self.elements_implicit)):
                    pass
                    if self._io.is_eof():
                        raise kaitaistruct.ConsistencyError(u"elements_implicit", 0, self._io.size() - self._io.pos())
                    self.elements_implicit[i]._write__seq(self._io)
        ...
        def _check(self):
            if (not (self.is_forced_implicit)):
                pass
                if len((self.vr).encode(u"ASCII")) != 2:
                    raise kaitaistruct.ConsistencyError(u"vr", 2, len((self.vr).encode(u"ASCII")))
            ...
            if  ((self.vr == u"SQ") and (self.value_len == 4294967295)) :
                ...
                for i in range(len(self.items)):
                    ...
                    if self.items[i]._parent != self:
                        raise kaitaistruct.ConsistencyError(u"items", self, self.items[i]._parent)
                    _ = self.items[i]
                    if (_.tag_elem == 57565) != (i == len(self.items) - 1):
                        raise kaitaistruct.ConsistencyError(u"items", i == len(self.items) - 1, _.tag_elem == 57565)
            ...
            if self.is_transfer_syntax_change_implicit:
                ...
                for i in range(len(self.elements_implicit)):
                    pass
                    if self.elements_implicit[i]._root != self._root:
                        raise kaitaistruct.ConsistencyError(u"elements_implicit", self._root, self.elements_implicit[i]._root)
                    if self.elements_implicit[i]._parent != self:
                        raise kaitaistruct.ConsistencyError(u"elements_implicit", self, self.elements_implicit[i]._parent)
        ...
        def _invalidate_is_forced_implicit(self):
            del self._m_is_forced_implicit
        @property
        ...
        def _invalidate_is_long_len(self):
            del self._m_is_long_len
        @property
        ...
        def _invalidate_is_transfer_syntax_change_implicit(self):
            del self._m_is_transfer_syntax_change_implicit
        @property
        def tag(self):
    ...
    class TDataElementImplicit(ReadWriteKaitaiStruct):
        """
        .. seealso::
           Source - https://dicom.nema.org/medical/dicom/current/output/html/part05.html#sect_7.1.2
        """
        ...
        def _write__seq(self, io=None):
            super(Dicom.TDataElementImplicit, self)._write__seq(io)
            self._io.write_u2le(self.tag_group)
            self._io.write_u2le(self.tag_elem)
            if self.is_forced_explicit:
                pass
        ...
        def _invalidate_is_forced_explicit(self):
            del self._m_is_forced_explicit
        @property
        def is_long_len(self):
            if hasattr(self, '_m_is_long_len'):
                return self._m_is_long_len
        ...
        def _invalidate_is_transfer_syntax_change_explicit(self):
            del self._m_is_transfer_syntax_change_explicit
        @property
        ...
        def _invalidate_p_is_transfer_syntax_change_explicit(self):
            del self._m_p_is_transfer_syntax_change_explicit
        @property
        def tag(self):
            if hasattr(self, '_m_tag'):
                return self._m_tag
    ...
    class TFileHeader(ReadWriteKaitaiStruct):
        def __init__(self, _io=None, _parent=None, _root=None):
        ...
        def _write__seq(self, io=None):
            super(Dicom.TFileHeader, self)._write__seq(io)
            self._io.write_bytes(self.preamble)
            self._io.write_bytes(self.magic)


        def _check(self):
            if len(self.preamble) != 128:
                raise kaitaistruct.ConsistencyError(u"preamble", 128, len(self.preamble))
            if len(self.magic) != 4:
                raise kaitaistruct.ConsistencyError(u"magic", 4, len(self.magic))
            if not self.magic == b"\x44\x49\x43\x4D":
                raise kaitaistruct.ValidationNotEqualError(b"\x44\x49\x43\x4D", self.magic, None, u"/types/t_file_header/seq/1")
            self._dirty = False
