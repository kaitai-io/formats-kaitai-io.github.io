...
function Dicom:_init(io, parent, root)
...
function Dicom:_read()
  self.file_header = Dicom.TFileHeader(self._io, self, self._root)
  self.elements = {}
  local i = 0
  while not self._io:is_eof() do
    self.elements[i + 1] = Dicom.TDataElementImplicit(self._io, self, self._root)
    i = i + 1
  end
end
...
function Dicom.TDataElementExplicit:_read()
  self.tag_group = self._io:read_u2le()
  self.tag_elem = self._io:read_u2le()
  if not(self.is_forced_implicit) then
    self.vr = str_decode.decode(self._io:read_bytes(2), "ASCII")
  end
  if  ((self.is_long_len) and (not(self.is_forced_implicit)))  then
    self.reserved = self._io:read_u2le()
  end
  local _on = self.is_long_len
  if _on == false then
