...
type
  Dicom* = ref object of KaitaiStruct
    `fileHeader`*: Dicom_TFileHeader
    `elements`*: seq[Dicom_TDataElementImplicit]
    `parent`*: KaitaiStruct
  Dicom_Tags* = enum
...
proc read*(_: typedesc[Dicom], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): Dicom =
  ...
  let fileHeaderExpr = Dicom_TFileHeader.read(this.io, this.root, this)
  this.fileHeader = fileHeaderExpr
  block:
    var i: int
    while not this.io.isEof:
      let it = Dicom_TDataElementImplicit.read(this.io, this.root, this)
...
proc read*(_: typedesc[Dicom_TDataElementExplicit], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): Dicom_TDataElementExplicit =
  template this: untyped = result
  this = new(Dicom_TDataElementExplicit)
  let root = if root == nil: cast[Dicom](this) else: cast[Dicom](root)
  this.io = io
  this.root = root
  this.parent = parent

  let tagGroupExpr = this.io.readU2le()
  this.tagGroup = tagGroupExpr
  let tagElemExpr = this.io.readU2le()
  this.tagElem = tagElemExpr
  if not(this.isForcedImplicit):
    let vrExpr = encode(this.io.readBytes(int(2)), "ASCII")
    this.vr = vrExpr
  if  ((this.isLongLen) and (not(this.isForcedImplicit))) :
    let reservedExpr = this.io.readU2le()
    this.reserved = reservedExpr
  block:
    let on = this.isLongLen
    if on == false:
      let valueLenExpr = uint32(this.io.readU2le())
  ...
  if  ((this.vr == "SQ") and (this.valueLen == 4294967295'i64)) :
    block:
      var i: int
      while true:
        let it = Dicom_SeqItem.read(this.io, this.root, this)
        this.items.add(it)
        if it.tagElem == 57565:
          break
        inc i
...
proc read*(_: typedesc[Dicom_TDataElementImplicit], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct): Dicom_TDataElementImplicit =
  template this: untyped = result
  this = new(Dicom_TDataElementImplicit)
  let root = if root == nil: cast[Dicom](this) else: cast[Dicom](root)
  this.io = io
  this.root = root
  this.parent = parent
  ...
  if this.isForcedExplicit:
    let vrExpr = encode(this.io.readBytes(int(2)), "ASCII")
    this.vr = vrExpr
  if  ((this.isLongLen) and (this.isForcedExplicit)) :
    let reservedExpr = this.io.readU2le()
    this.reserved = reservedExpr
  block:
    let on = (if this.isForcedExplicit: this.isLongLen else: true)
    if on == false:
      let valueLenExpr = uint32(this.io.readU2le())
      this.valueLen = valueLenExpr
    elif on == true:
