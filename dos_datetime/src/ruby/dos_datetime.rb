...
class DosDatetime < Kaitai::Struct::Struct
  ...
  class Date < Kaitai::Struct::Struct
    ...
    def padded_day
      ...
      @padded_day = (day <= 9 ? "0" : "") + day.to_s
      @padded_day
    end
    def padded_month
      return @padded_month unless @padded_month.nil?
      @padded_month = (month <= 9 ? "0" : "") + month.to_s
      @padded_month
    end
    def padded_year
      return @padded_year unless @padded_year.nil?
      @padded_year = (year <= 999 ? "0" + (year <= 99 ? "0" + (year <= 9 ? "0" : "") : "") : "") + year.to_s
      @padded_year
    end
  ...
  class Time < Kaitai::Struct::Struct
    ...
    def padded_hour
      ...
      @padded_hour = (hour <= 9 ? "0" : "") + hour.to_s
      @padded_hour
    end
    def padded_minute
      return @padded_minute unless @padded_minute.nil?
      @padded_minute = (minute <= 9 ? "0" : "") + minute.to_s
      @padded_minute
    end
    def padded_second
      return @padded_second unless @padded_second.nil?
      @padded_second = (second <= 9 ? "0" : "") + second.to_s
      @padded_second
    end
    def second
