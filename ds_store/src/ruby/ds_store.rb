...
class DsStore < Kaitai::Struct::Struct
  ...
  class Block < Kaitai::Struct::Struct
    ...
    class BlockData < Kaitai::Struct::Struct
      def initialize(_io, _parent = nil, _root = nil, mode)
        super(_io, _parent, _root)
        @mode = mode
        _read
      end
      ...
      class Record < Kaitai::Struct::Struct
        ...
        class Ustr < Kaitai::Struct::Struct
          ...
          def _read
            @length = @_io.read_u4be
            @value = (@_io.read_bytes(2 * length)).force_encoding("UTF-16BE").encode('UTF-8')
            self
          end
          attr_reader :length
          attr_reader :value
  ...
  class MasterBlockRef < Kaitai::Struct::Struct
    ...
    def master_block
      ...
      _io_master_block = @_io.substream(_parent.block_addresses[_parent.directory_entries[idx].block_id].size)
      @master_block = MasterBlock.new(_io_master_block, self, @_root)
      @_io.seek(_pos)
      @master_block
    end
    attr_reader :idx
  ...
  def buddy_allocator_body
    ...
    _io_buddy_allocator_body = @_io.substream(buddy_allocator_header.len_bookkeeping_info_block)
    @buddy_allocator_body = BuddyAllocatorBody.new(_io_buddy_allocator_body, self, @_root)
    @_io.seek(_pos)
