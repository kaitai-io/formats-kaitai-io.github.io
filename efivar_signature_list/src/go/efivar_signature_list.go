...
func (this EfivarSignatureList) IO_() *kaitai.Stream {
	return this._io
}

func (this *EfivarSignatureList) Read(io *kaitai.Stream, parent kaitai.Struct, root *EfivarSignatureList) (err error) {
	this._io = io
	this._parent = parent
	this._root = root
...
func (this EfivarSignatureList_EfiVarAttr) IO_() *kaitai.Stream {
	return this._io
}
...
func (this *EfivarSignatureList_EfiVarAttr) Read(io *kaitai.Stream, parent *EfivarSignatureList, root *EfivarSignatureList) (err error) {
	...
	this.EnhancedAuthenticatedAccess = tmp4 != 0
	tmp5, err := this._io.ReadBitsIntBe(1)
	if err != nil {
		return err
	}
	this.AppendWrite = tmp5 != 0
	tmp6, err := this._io.ReadBitsIntBe(1)
	if err != nil {
		return err
	}
	this.TimeBasedAuthenticatedWriteAccess = tmp6 != 0
	tmp7, err := this._io.ReadBitsIntBe(1)
	if err != nil {
		return err
	}
	this.AuthenticatedWriteAccess = tmp7 != 0
	tmp8, err := this._io.ReadBitsIntBe(1)
	if err != nil {
		return err
	}
	this.HardwareErrorRecord = tmp8 != 0
	tmp9, err := this._io.ReadBitsIntBe(1)
	if err != nil {
		return err
	}
	this.RuntimeAccess = tmp9 != 0
	tmp10, err := this._io.ReadBitsIntBe(1)
	if err != nil {
		return err
	}
	this.BootserviceAccess = tmp10 != 0
	tmp11, err := this._io.ReadBitsIntBe(1)
	if err != nil {
		return err
	}
	this.NonVolatile = tmp11 != 0
	tmp12, err := this._io.ReadBitsIntBe(24)
	if err != nil {
		return err
	}
...
func (this EfivarSignatureList_SignatureData) IO_() *kaitai.Stream {
	return this._io
}
...
func (this EfivarSignatureList_SignatureList) IO_() *kaitai.Stream {
	return this._io
}
...
func (this *EfivarSignatureList_SignatureList) Read(io *kaitai.Stream, parent *EfivarSignatureList, root *EfivarSignatureList) (err error) {
	...
	tmp15 = tmp15
	this.SignatureType = tmp15
	tmp16, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.LenSignatureList = uint32(tmp16)
	tmp17, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.LenSignatureHeader = uint32(tmp17)
	tmp18, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.LenSignature = uint32(tmp18)
	tmp19, err := this._io.ReadBytes(int(this.LenSignatureHeader))
	if err != nil {
		return err
	}
	tmp19 = tmp19
	...
	if (this.LenSignature > 0) {
		for i := 0; i < int(((this.LenSignatureList - this.LenSignatureHeader) - 28) / this.LenSignature); i++ {
			_ = i
			tmp20, err := this._io.ReadBytes(int(this.LenSignature))
			if err != nil {
				return err
			}
			tmp20 = tmp20
			this._raw_Signatures = append(this._raw_Signatures, tmp20)
			_io__raw_Signatures := kaitai.NewStream(bytes.NewReader(this._raw_Signatures[i]))
			tmp21 := NewEfivarSignatureList_SignatureData()
			err = tmp21.Read(_io__raw_Signatures, this, this._root)
			if err != nil {
				return err
			}
			this.Signatures = append(this.Signatures, tmp21)
		}
	}
	return err
}
