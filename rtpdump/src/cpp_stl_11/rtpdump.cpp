...
void rtpdump_t::header_t_t::_read() {
    ...
    if (!(m_shebang == std::string("\x23\x21\x72\x74\x70\x70\x6C\x61\x79\x31\x2E\x30", 12))) {
        throw kaitai::validation_not_equal_error<std::string>(std::string("\x23\x21\x72\x74\x70\x70\x6C\x61\x79\x31\x2E\x30", 12), m_shebang, m__io, std::string("/types/header_t/seq/0"));
    }
    m_space = m__io->read_bytes(1);
    if (!(m_space == std::string("\x20", 1))) {
        throw kaitai::validation_not_equal_error<std::string>(std::string("\x20", 1), m_space, m__io, std::string("/types/header_t/seq/1"));
    }
    m_ip = kaitai::kstream::bytes_to_str(m__io->read_bytes_term(47, false, true, true), "ASCII");
    m_port = kaitai::kstream::bytes_to_str(m__io->read_bytes_term(10, false, true, true), "ASCII");
