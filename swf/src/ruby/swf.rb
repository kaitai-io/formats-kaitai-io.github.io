  ...
  class Tag < Kaitai::Struct::Struct
    ...
    def _read
      ...
      when :tag_type_define_sound
        _io_tag_body = @_io.substream(record_header.len)
        @tag_body = DefineSoundBody.new(_io_tag_body, self, @_root)
      when :tag_type_do_abc
        _io_tag_body = @_io.substream(record_header.len)
        @tag_body = DoAbcBody.new(_io_tag_body, self, @_root)
      when :tag_type_export_assets
        _io_tag_body = @_io.substream(record_header.len)
        @tag_body = SymbolClassBody.new(_io_tag_body, self, @_root)
      when :tag_type_script_limits
        _io_tag_body = @_io.substream(record_header.len)
        @tag_body = ScriptLimitsBody.new(_io_tag_body, self, @_root)
      when :tag_type_set_background_color
        _io_tag_body = @_io.substream(record_header.len)
        @tag_body = Rgb.new(_io_tag_body, self, @_root)
