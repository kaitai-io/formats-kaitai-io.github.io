...
void swf_t::_read() {
    ...
    m_signature = m__io->ensure_fixed_contents(std::string("\x57\x53", 2));
...
swf_t::~swf_t() {
    if (!n_plain_body) {
        delete m__io__raw_plain_body;
        delete m_plain_body;
    }
    if (!n_zlib_body) {
        delete m__io__raw_zlib_body;
        delete m_zlib_body;
    }
}
...
void swf_t::tag_t::_read() {
    ...
    case TAG_TYPE_SET_BACKGROUND_COLOR: {
        n_tag_body = false;
...
void swf_t::define_sound_body_t::_read() {
    ...
    m_format = m__io->read_bits_int(4);
    m_sampling_rate = static_cast<swf_t::define_sound_body_t::sampling_rates_t>(m__io->read_bits_int(2));
    m_bits_per_sample = static_cast<swf_t::define_sound_body_t::bps_t>(m__io->read_bits_int(1));
    m_num_channels = static_cast<swf_t::define_sound_body_t::channels_t>(m__io->read_bits_int(1));
    m__io->align_to_byte();
