...
sub _read {
    ...
    if ($self->format() == 1) {
        ...
        for (my $i = 0; $i < $n_data_formats; $i++) {
            $self->{data_formats}[$i] = MacOsResourceSnd::DataFormat->new($self->{_io}, $self, $self->{_root});
        }
    }
    if ($self->format() == 2) {
    ...
    for (my $i = 0; $i < $n_sound_commands; $i++) {
        $self->{sound_commands}[$i] = MacOsResourceSnd::SoundCommand->new($self->{_io}, $self, $self->{_root});
    }
}
