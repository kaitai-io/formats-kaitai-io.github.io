...
public class FasttrackerXmModule extends KaitaiStruct {
    ...
    private void _read() {
        ...
        patterns = new ArrayList<Pattern>((int) (header().numPatterns()));
        for (int i = 0; i < header().numPatterns(); i++) {
            this.patterns.add(new Pattern(this._io, this, _root));
        }
        instruments = new ArrayList<Instrument>((int) (header().numInstruments()));
    ...
    public static class Instrument extends KaitaiStruct {
        ...
        private void _read() {
            ...
            KaitaiStream _io__raw_header = new ByteBufferKaitaiStream(_raw_header);
            this.header = new Header(_io__raw_header, this, _root);
            samplesHeaders = new ArrayList<SampleHeader>((int) (header().numSamples()));
            for (int i = 0; i < header().numSamples(); i++) {
                this.samplesHeaders.add(new SampleHeader(this._io, this, _root));
            }
            samples = new ArrayList<SamplesData>((int) (header().numSamples()));
            for (int i = 0; i < header().numSamples(); i++) {
        ...
        public static class ExtraHeader extends KaitaiStruct {
            ...
            private void _read() {
                ...
                volumePoints = new ArrayList<EnvelopePoint>((int) (12));
                for (int i = 0; i < 12; i++) {
                    this.volumePoints.add(new EnvelopePoint(this._io, this, _root));
                }
                panningPoints = new ArrayList<EnvelopePoint>((int) (12));
                for (int i = 0; i < 12; i++) {
        ...
        public static class SamplesData extends KaitaiStruct {
            ...
            public SamplesData(KaitaiStream _io, FasttrackerXmModule.Instrument _parent, int index) {
                this(_io, _parent, null, index);
            }

            public SamplesData(KaitaiStream _io, FasttrackerXmModule.Instrument _parent, FasttrackerXmModule _root, int index) {
                super(_io);
                this._parent = _parent;
                this._root = _root;
                this.index = index;
