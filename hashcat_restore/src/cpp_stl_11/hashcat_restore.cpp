...
void hashcat_restore_t::_read() {
    ...
    m_cwd = kaitai::kstream::bytes_to_str(kaitai::kstream::bytes_terminate(m__io->read_bytes(256), 0, false), "UTF-8");
    ...
    for (int i = 0; i < l_argv; i++) {
        m_argv->push_back(std::move(kaitai::kstream::bytes_to_str(m__io->read_bytes_term(10, false, true, true), "UTF-8")));
    }
}
