...
class MachO < Kaitai::Struct::Struct
  ...
  class CodeSignatureCommand < Kaitai::Struct::Struct
    ...
    def code_signature
      ...
      _pos = io.pos
      io.seek(data_off)
      _io_code_signature = io.substream(data_size)
      @code_signature = CsBlob.new(_io_code_signature, self, @_root)
      io.seek(_pos)
      @code_signature
    end
  ...
  class CsBlob < Kaitai::Struct::Struct
    ...
    def _read
      ...
      when :cs_magic_blob_wrapper
        _io_body = @_io.substream(length - 8)
        @body = BlobWrapper.new(_io_body, self, @_root)
      when :cs_magic_code_directory
        _io_body = @_io.substream(length - 8)
        @body = CodeDirectory.new(_io_body, self, @_root)
      when :cs_magic_der_entitlements
        _io_body = @_io.substream(length - 8)
        @body = Asn1Der.new(_io_body)
      when :cs_magic_detached_signature
        _io_body = @_io.substream(length - 8)
        @body = SuperBlob.new(_io_body, self, @_root)
      when :cs_magic_embedded_signature
        _io_body = @_io.substream(length - 8)
        @body = SuperBlob.new(_io_body, self, @_root)
      when :cs_magic_entitlements
        _io_body = @_io.substream(length - 8)
        @body = Entitlements.new(_io_body, self, @_root)
      when :cs_magic_requirement
        _io_body = @_io.substream(length - 8)
        @body = Requirement.new(_io_body, self, @_root)
      when :cs_magic_requirements
        _io_body = @_io.substream(length - 8)
        @body = Requirements.new(_io_body, self, @_root)
      else
  ...
  class DyldInfoCommand < Kaitai::Struct::Struct
    ...
    class BindItem < Kaitai::Struct::Struct
      ...
      def _read
        ...
        if opcode == :bind_opcode_set_symbol_trailing_flags_immediate
          @symbol = (@_io.read_bytes_term(0, false, true, true)).force_encoding("ASCII").encode('UTF-8')
        end
        self
      end
      def immediate
    ...
    def bind
      ...
      if bind_size != 0
        ...
        _pos = io.pos
        io.seek(bind_off)
        _io_bind = io.substream(bind_size)
        @bind = BindData.new(_io_bind, self, @_root)
        io.seek(_pos)
      end
      @bind
    end
    def exports
      ...
      if export_size != 0
        ...
        _pos = io.pos
        io.seek(export_off)
        _io_exports = io.substream(export_size)
        @exports = ExportNode.new(_io_exports, self, @_root)
        io.seek(_pos)
      end
      @exports
    end
    def lazy_bind
      ...
      if lazy_bind_size != 0
        ...
        _io_lazy_bind = io.substream(lazy_bind_size)
        @lazy_bind = BindData.new(_io_lazy_bind, self, @_root)
        io.seek(_pos)
      end
      @lazy_bind
    end
    def rebase
      ...
      if rebase_size != 0
        ...
        _pos = io.pos
        io.seek(rebase_off)
        _io_rebase = io.substream(rebase_size)
        @rebase = RebaseData.new(_io_rebase, self, @_root)
        io.seek(_pos)
      end
      @rebase
    end
    def weak_bind
      ...
      if weak_bind_size != 0
        ...
        _io_weak_bind = io.substream(weak_bind_size)
        @weak_bind = BindData.new(_io_weak_bind, self, @_root)
        io.seek(_pos)
      end
      @weak_bind
    end
  ...
  class LoadCommand < Kaitai::Struct::Struct
    ...
    def _read
      ...
      when :load_command_type_build_version
        _io_body = @_io.substream(size - 8)
        @body = BuildVersionCommand.new(_io_body, self, @_root)
      when :load_command_type_code_signature
        _io_body = @_io.substream(size - 8)
        @body = CodeSignatureCommand.new(_io_body, self, @_root)
      when :load_command_type_data_in_code
        _io_body = @_io.substream(size - 8)
        @body = LinkeditDataCommand.new(_io_body, self, @_root)
      when :load_command_type_dyld_environment
        _io_body = @_io.substream(size - 8)
        @body = DylinkerCommand.new(_io_body, self, @_root)
      when :load_command_type_dyld_info
        _io_body = @_io.substream(size - 8)
        @body = DyldInfoCommand.new(_io_body, self, @_root)
      ...
      when :load_command_type_dysymtab
        _io_body = @_io.substream(size - 8)
        @body = DysymtabCommand.new(_io_body, self, @_root)
      when :load_command_type_encryption_info
        _io_body = @_io.substream(size - 8)
        @body = EncryptionInfoCommand.new(_io_body, self, @_root)
      ...
      when :load_command_type_id_dylib
        _io_body = @_io.substream(size - 8)
        @body = DylibCommand.new(_io_body, self, @_root)
      ...
      when :load_command_type_linker_option
        _io_body = @_io.substream(size - 8)
        @body = LinkerOptionCommand.new(_io_body, self, @_root)
      ...
      when :load_command_type_main
        _io_body = @_io.substream(size - 8)
        @body = EntryPointCommand.new(_io_body, self, @_root)
      when :load_command_type_reexport_dylib
        _io_body = @_io.substream(size - 8)
        @body = DylibCommand.new(_io_body, self, @_root)
      when :load_command_type_routines
        _io_body = @_io.substream(size - 8)
        @body = RoutinesCommand.new(_io_body, self, @_root)
      when :load_command_type_routines_64
        _io_body = @_io.substream(size - 8)
        @body = RoutinesCommand64.new(_io_body, self, @_root)
      when :load_command_type_rpath
        _io_body = @_io.substream(size - 8)
        @body = RpathCommand.new(_io_body, self, @_root)
      when :load_command_type_segment
        _io_body = @_io.substream(size - 8)
        @body = SegmentCommand.new(_io_body, self, @_root)
      when :load_command_type_segment_64
        _io_body = @_io.substream(size - 8)
        @body = SegmentCommand64.new(_io_body, self, @_root)
      when :load_command_type_segment_split_info
        _io_body = @_io.substream(size - 8)
        @body = LinkeditDataCommand.new(_io_body, self, @_root)
      when :load_command_type_source_version
        _io_body = @_io.substream(size - 8)
        @body = SourceVersionCommand.new(_io_body, self, @_root)
      when :load_command_type_sub_client
        _io_body = @_io.substream(size - 8)
        @body = SubCommand.new(_io_body, self, @_root)
      ...
      when :load_command_type_symtab
        _io_body = @_io.substream(size - 8)
        @body = SymtabCommand.new(_io_body, self, @_root)
      when :load_command_type_twolevel_hints
        _io_body = @_io.substream(size - 8)
        @body = TwolevelHintsCommand.new(_io_body, self, @_root)
      when :load_command_type_uuid
        _io_body = @_io.substream(size - 8)
        @body = UuidCommand.new(_io_body, self, @_root)
      when :load_command_type_version_min_iphoneos
        _io_body = @_io.substream(size - 8)
        @body = VersionMinCommand.new(_io_body, self, @_root)
  ...
  class SegmentCommand < Kaitai::Struct::Struct
    ...
    def _read
      @segname = (Kaitai::Struct::Stream::bytes_strip_right(@_io.read_bytes(16), 0)).force_encoding("ASCII").encode('UTF-8')
    ...
    class Section < Kaitai::Struct::Struct
      ...
      def _read
        @sect_name = (Kaitai::Struct::Stream::bytes_strip_right(@_io.read_bytes(16), 0)).force_encoding("ASCII").encode('UTF-8')
        @seg_name = (Kaitai::Struct::Stream::bytes_strip_right(@_io.read_bytes(16), 0)).force_encoding("ASCII").encode('UTF-8')
        @addr = @_io.read_u4le
        @size = @_io.read_u4le
  ...
  class SegmentCommand64 < Kaitai::Struct::Struct
    ...
    class Section64 < Kaitai::Struct::Struct
      ...
      class EhFrameItem < Kaitai::Struct::Struct
        ...
        def _read
          ...
          if length > 0
            case id
            when 0
              _io_body = @_io.substream(length - 4)
              @body = Cie.new(_io_body, self, @_root)
            else
      ...
      def data
        ...
        case sect_name
        when "__cfstring"
          _io_data = io.substream(size)
          @data = CfStringList.new(_io_data, self, @_root)
        when "__cstring"
          _io_data = io.substream(size)
          @data = StringList.new(_io_data, self, @_root)
        when "__eh_frame"
          _io_data = io.substream(size)
          @data = EhFrame.new(_io_data, self, @_root)
        when "__got"
          _io_data = io.substream(size)
          @data = PointerList.new(_io_data, self, @_root)
  ...
  class SymtabCommand < Kaitai::Struct::Struct
    ...
    def strs
      ...
      _pos = io.pos
      io.seek(str_off)
      _io_strs = io.substream(str_size)
      @strs = StrTable.new(_io_strs, self, @_root)
      io.seek(_pos)
      @strs
    end
    def symbols
