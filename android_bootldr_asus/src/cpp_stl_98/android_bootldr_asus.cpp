...
void android_bootldr_asus_t::_read() {
    ...
    if (!(m_revision >= 2)) {
        throw kaitai::validation_less_than_error<uint16_t>(2, m_revision, m__io, std::string("/seq/1"));
    }
...
void android_bootldr_asus_t::image_t::_read() {
    m_chunk_id = kaitai::kstream::bytes_to_str(m__io->read_bytes(8), "ASCII");
    if (!( ((m_chunk_id == std::string("IFWI!!!!")) || (m_chunk_id == std::string("DROIDBT!")) || (m_chunk_id == std::string("SPLASHS!"))) )) {
        throw kaitai::validation_not_any_of_error<std::string>(m_chunk_id, m__io, std::string("/types/image/seq/0"));
    }
    ...
    m_flags = m__io->read_u1();
    {
        uint8_t _ = m_flags;
        if (!((_ & 1) != 0)) {
            throw kaitai::validation_expr_error<uint8_t>(m_flags, m__io, std::string("/types/image/seq/2"));
        }
    }
