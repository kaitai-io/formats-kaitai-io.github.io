...
class AndroidSuper < Kaitai::Struct::Struct
  ...
  class Metadata < Kaitai::Struct::Struct
    ...
    class TableDescriptor < Kaitai::Struct::Struct
      def initialize(_io, _parent = nil, _root = nil, kind)
        super(_io, _parent, _root)
        @kind = kind
        _read
      end
      ...
      def table
        ...
        (num_entries).times { |i|
          ...
          when :table_kind_block_devices
            _io_table = @_io.substream(entry_size)
            @table << BlockDevice.new(_io_table, self, @_root)
          when :table_kind_extents
            _io_table = @_io.substream(entry_size)
            @table << Extent.new(_io_table, self, @_root)
          when :table_kind_groups
            _io_table = @_io.substream(entry_size)
            @table << Group.new(_io_table, self, @_root)
          when :table_kind_partitions
            _io_table = @_io.substream(entry_size)
            @table << Partition.new(_io_table, self, @_root)
          else
  ...
  class Root < Kaitai::Struct::Struct
    ...
    def _read
      _io_primary_geometry = @_io.substream(4096)
      @primary_geometry = Geometry.new(_io_primary_geometry, self, @_root)
      _io_backup_geometry = @_io.substream(4096)
      @backup_geometry = Geometry.new(_io_backup_geometry, self, @_root)
      @_raw_primary_metadata = []
      @primary_metadata = []
      (primary_geometry.metadata_slot_count).times { |i|
        _io_primary_metadata = @_io.substream(primary_geometry.metadata_max_size)
        @primary_metadata << Metadata.new(_io_primary_metadata, self, @_root)
      }
      @_raw_backup_metadata = []
      @backup_metadata = []
      (primary_geometry.metadata_slot_count).times { |i|
        _io_backup_metadata = @_io.substream(primary_geometry.metadata_max_size)
        @backup_metadata << Metadata.new(_io_backup_metadata, self, @_root)
      }
      self
    end
