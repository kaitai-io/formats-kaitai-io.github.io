...
class Id3v11(ReadWriteKaitaiStruct):
    ...
    def __init__(self, _io=None, _parent=None, _root=None):
        ...
        self._should_write_id3v1_tag = False
        self.id3v1_tag__enabled = True
    ...
    def _write__seq(self, io=None):
        super(Id3v11, self)._write__seq(io)
        self._should_write_id3v1_tag = self.id3v1_tag__enabled


    def _check(self):
        if self.id3v1_tag__enabled:
            pass
            if self._m_id3v1_tag._root != self._root:
                raise kaitaistruct.ConsistencyError(u"id3v1_tag", self._root, self._m_id3v1_tag._root)
            if self._m_id3v1_tag._parent != self:
                raise kaitaistruct.ConsistencyError(u"id3v1_tag", self, self._m_id3v1_tag._parent)

        self._dirty = False

    class Id3V11Tag(ReadWriteKaitaiStruct):
        ...
        def _write__seq(self, io=None):
            super(Id3v11.Id3V11Tag, self)._write__seq(io)
            self._io.write_bytes(self.magic)
            self._io.write_bytes(self.title)
            self._io.write_bytes(self.artist)
            self._io.write_bytes(self.album)
            self._io.write_bytes((self.year).encode(u"ASCII"))
            self._io.write_bytes(self.comment)
            self._io.write_u1(int(self.genre))
        ...
        def _check(self):
            ...
            if not self.magic == b"\x54\x41\x47":
                raise kaitaistruct.ValidationNotEqualError(b"\x54\x41\x47", self.magic, None, u"/types/id3_v1_1_tag/seq/0")
            if len(self.title) != 30:
                raise kaitaistruct.ConsistencyError(u"title", 30, len(self.title))
            if len(self.artist) != 30:
                raise kaitaistruct.ConsistencyError(u"artist", 30, len(self.artist))
            if len(self.album) != 30:
                raise kaitaistruct.ConsistencyError(u"album", 30, len(self.album))
            ...
            if len(self.comment) != 30:
                raise kaitaistruct.ConsistencyError(u"comment", 30, len(self.comment))
            self._dirty = False


    @property
    def id3v1_tag(self):
        if self._should_write_id3v1_tag:
            self._write_id3v1_tag()
        if hasattr(self, '_m_id3v1_tag'):
            return self._m_id3v1_tag

        if not self.id3v1_tag__enabled:
            return None
    ...
    @id3v1_tag.setter
    def id3v1_tag(self, v):
        self._dirty = True
        self._m_id3v1_tag = v

    def _write_id3v1_tag(self):
        self._should_write_id3v1_tag = False
        _pos = self._io.pos()
        self._io.seek(self._io.size() - 128)
        self._m_id3v1_tag._write__seq(self._io)
        self._io.seek(_pos)
