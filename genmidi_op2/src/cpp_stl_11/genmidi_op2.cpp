...
void genmidi_op2_t::_read() {
    ...
    if (!(m_magic == std::string("\x23\x4F\x50\x4C\x5F\x49\x49\x23", 8))) {
        throw kaitai::validation_not_equal_error<std::string>(std::string("\x23\x4F\x50\x4C\x5F\x49\x49\x23", 8), m_magic, m__io, std::string("/seq/0"));
    }
    ...
    for (int i = 0; i < l_instrument_names; i++) {
        m_instrument_names->push_back(std::move(kaitai::kstream::bytes_to_str(kaitai::kstream::bytes_terminate(kaitai::kstream::bytes_strip_right(m__io->read_bytes(32), 0), 0, false), "ASCII")));
    }
}
