...
class Mcap < Kaitai::Struct::Struct
  ...
  class AttachmentIndex < Kaitai::Struct::Struct
    ...
    def attachment
      ...
      _io_attachment = io.substream(len_attachment)
      @attachment = Record.new(_io_attachment, self, @_root)
      io.seek(_pos)
      @attachment
    end
  ...
  class Chunk < Kaitai::Struct::Struct
    ...
    def _read
      ...
      when ""
        _io_records = @_io.substream(len_records)
        @records = Records.new(_io_records, self, @_root)
      else
  ...
  class ChunkIndex < Kaitai::Struct::Struct
    ...
    def _read
      ...
      _io_message_index_offsets = @_io.substream(len_message_index_offsets)
      @message_index_offsets = MessageIndexOffsets.new(_io_message_index_offsets, self, @_root)
    ...
    def chunk
      ...
      _pos = io.pos
      io.seek(ofs_chunk)
      _io_chunk = io.substream(len_chunk)
      @chunk = Record.new(_io_chunk, self, @_root)
      io.seek(_pos)
      @chunk
    end
    attr_reader :message_start_time
  ...
  class Footer < Kaitai::Struct::Struct
    ...
    def summary_offset_section
      ...
      if ofs_summary_offset_section != 0
        ...
        _io_summary_offset_section = io.substream(_root.ofs_footer - ofs_summary_offset_section)
        @summary_offset_section = Records.new(_io_summary_offset_section, self, @_root)
        io.seek(_pos)
      end
    ...
    def summary_section
      ...
      if ofs_summary_section != 0
        ...
        _io_summary_section = io.substream((ofs_summary_offset_section != 0 ? ofs_summary_offset_section : _root.ofs_footer) - ofs_summary_section)
        @summary_section = Records.new(_io_summary_section, self, @_root)
        io.seek(_pos)
      end
      @summary_section
    end
  ...
  class MapStrStr < Kaitai::Struct::Struct
    ...
    def _read
      ...
      @entries = Entries.new(_io_entries, self, @_root)
      self
    end
  ...
  class MessageIndex < Kaitai::Struct::Struct
    ...
    def _read
      ...
      _io_records = @_io.substream(len_records)
      @records = MessageIndexEntries.new(_io_records, self, @_root)
      self
    end
  ...
  class MetadataIndex < Kaitai::Struct::Struct
    ...
    def metadata
      ...
      _io_metadata = io.substream(len_metadata)
      @metadata = Record.new(_io_metadata, self, @_root)
      io.seek(_pos)
      @metadata
    end
  ...
  class Record < Kaitai::Struct::Struct
    ...
    def _read
      ...
      when :opcode_attachment
        ...
        @body = Attachment.new(_io_body, self, @_root)
      when :opcode_attachment_index
        _io_body = @_io.substream(len_body)
        @body = AttachmentIndex.new(_io_body, self, @_root)
      when :opcode_channel
        _io_body = @_io.substream(len_body)
        @body = Channel.new(_io_body, self, @_root)
      when :opcode_chunk
        _io_body = @_io.substream(len_body)
        @body = Chunk.new(_io_body, self, @_root)
      when :opcode_chunk_index
        _io_body = @_io.substream(len_body)
        @body = ChunkIndex.new(_io_body, self, @_root)
      when :opcode_data_end
        _io_body = @_io.substream(len_body)
        @body = DataEnd.new(_io_body, self, @_root)
      when :opcode_footer
        _io_body = @_io.substream(len_body)
        @body = Footer.new(_io_body, self, @_root)
      when :opcode_header
        _io_body = @_io.substream(len_body)
        @body = Header.new(_io_body, self, @_root)
      when :opcode_message
        _io_body = @_io.substream(len_body)
        @body = Message.new(_io_body, self, @_root)
      when :opcode_message_index
        _io_body = @_io.substream(len_body)
        @body = MessageIndex.new(_io_body, self, @_root)
      when :opcode_metadata
        _io_body = @_io.substream(len_body)
        @body = Metadata.new(_io_body, self, @_root)
      when :opcode_metadata_index
        _io_body = @_io.substream(len_body)
        @body = MetadataIndex.new(_io_body, self, @_root)
      when :opcode_schema
        _io_body = @_io.substream(len_body)
        @body = Schema.new(_io_body, self, @_root)
      when :opcode_statistics
        _io_body = @_io.substream(len_body)
        @body = Statistics.new(_io_body, self, @_root)
      when :opcode_summary_offset
        _io_body = @_io.substream(len_body)
        @body = SummaryOffset.new(_io_body, self, @_root)
      else
  ...
  class Statistics < Kaitai::Struct::Struct
    ...
    def _read
      ...
      _io_channel_message_counts = @_io.substream(len_channel_message_counts)
      @channel_message_counts = ChannelMessageCounts.new(_io_channel_message_counts, self, @_root)
      self
    end
  ...
  class SummaryOffset < Kaitai::Struct::Struct
    ...
    def group
      ...
      _pos = io.pos
      io.seek(ofs_group)
      _io_group = io.substream(len_group)
      @group = Records.new(_io_group, self, @_root)
      io.seek(_pos)
      @group
    end
