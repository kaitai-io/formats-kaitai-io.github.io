...
void vmware_vmdk_t::_read() {
    m_magic = m__io->ensure_fixed_contents(std::string("\x4B\x44\x4D\x56", 4));
...
void vmware_vmdk_t::header_flags_t::_read() {
    ...
    m_zeroed_grain_table_entry = m__io->read_bits_int(1);
    m_use_secondary_grain_dir = m__io->read_bits_int(1);
    m_valid_new_line_detection_test = m__io->read_bits_int(1);
    m__io->align_to_byte();
    m_reserved2 = m__io->read_u1();
    m_reserved3 = m__io->read_bits_int(6);
    m_has_metadata = m__io->read_bits_int(1);
    m_has_compressed_grain = m__io->read_bits_int(1);
    m__io->align_to_byte();
