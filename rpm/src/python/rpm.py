    ...
    * v6, written by upstream RPM 6.0 by default - see
    ...
    def _read(self):
        ...
        for i in range(self.signature.header_record.num_index_records):
            self.signature_tags_steps.append(Rpm.SignatureTagsStep(i, (self.signature_tags_steps[i - 1].size_tag_idx if i != 0 else -1), (self.signature_tags_steps[i - 1].long_size_tag_idx if i != 0 else -1), self._io, self, self._root))

        self.header_tags_steps = []
        for i in range(self.header.header_record.num_index_records):
            self.header_tags_steps.append(Rpm.HeaderTagsStep(i, (self.header_tags_steps[i - 1].payload_size_tag_idx if i != 0 else -1), self._io, self, self._root))
    ...
    class SignatureTagsStep(KaitaiStruct):
        ...
        parameters.
        """
        def __init__(self, idx, prev_size_tag_idx, prev_long_size_tag_idx, _io, _parent=None, _root=None):
