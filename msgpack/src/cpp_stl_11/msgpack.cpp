...
void msgpack_t::_read() {
    ...
    if (is_array()) {
        ...
        for (int i = 0; i < l_array_elements; i++) {
            m_array_elements->push_back(std::move(std::unique_ptr<msgpack_t>(new msgpack_t(m__io))));
        }
    }
...
void msgpack_t::map_tuple_t::_read() {
    m_key = std::unique_ptr<msgpack_t>(new msgpack_t(m__io));
    m_value = std::unique_ptr<msgpack_t>(new msgpack_t(m__io));
}
