...
sub _read {
    ...
    if ($self->is_array()) {
        ...
        for (my $i = 0; $i < $n_array_elements; $i++) {
            push @{$self->{array_elements}}, Msgpack->new($self->{_io}, $self, $self->{_root});
        }
    }
    if ($self->is_map_16()) {
...
sub _read {
    ...
    $self->{key} = Msgpack->new($self->{_io}, $self, $self->{_root});
    $self->{value} = Msgpack->new($self->{_io}, $self, $self->{_root});
}
