...
class Iso9660 < Kaitai::Struct::Struct
  ...
  class DecDatetime < Kaitai::Struct::Struct
    ...
    def _read
      ...
      @month = (@_io.read_bytes(2)).force_encoding("ASCII").encode('UTF-8')
      @day = (@_io.read_bytes(2)).force_encoding("ASCII").encode('UTF-8')
      @hour = (@_io.read_bytes(2)).force_encoding("ASCII").encode('UTF-8')
      @minute = (@_io.read_bytes(2)).force_encoding("ASCII").encode('UTF-8')
      @sec = (@_io.read_bytes(2)).force_encoding("ASCII").encode('UTF-8')
      @sec_hundreds = (@_io.read_bytes(2)).force_encoding("ASCII").encode('UTF-8')
      @timezone = @_io.read_u1
      self
    end
    attr_reader :year
  ...
  class DirEntry < Kaitai::Struct::Struct
    ...
    def _read
      @len = @_io.read_u1
      if len > 0
        _io_body = @_io.substream(len - 1)
        @body = DirEntryBody.new(_io_body, self, @_root)
      end
      self
    end
    attr_reader :len
  ...
  class DirEntryBody < Kaitai::Struct::Struct
    ...
    def extent_as_dir
      ...
      if file_flags & 2 != 0
        ...
        _io_extent_as_dir = io.substream(size_extent.le)
        @extent_as_dir = DirEntries.new(_io_extent_as_dir, self, @_root)
        io.seek(_pos)
      end
      @extent_as_dir
    end
  ...
  class VolDescPrimary < Kaitai::Struct::Struct
    ...
    def _read
      ...
      _io_root_dir = @_io.substream(34)
      @root_dir = DirEntry.new(_io_root_dir, self, @_root)
    ...
    def path_table
      ...
      _io_path_table = @_io.substream(path_table_size.le)
      @path_table = PathTableLe.new(_io_path_table, self, @_root)
      @_io.seek(_pos)
      @path_table
    end
    attr_reader :unused1
