...
void s3m_t::_read() {
    ...
    m_magic1 = m__io->ensure_fixed_contents(std::string("\x1A", 1));
    ...
    m_magic2 = m__io->ensure_fixed_contents(std::string("\x53\x43\x52\x4D", 4));
    m_global_volume = m__io->read_u1();
    m_initial_speed = m__io->read_u1();
    m_initial_tempo = m__io->read_u1();
    m_is_stereo = m__io->read_bits_int(1);
    m_master_volume = m__io->read_bits_int(7);
    m__io->align_to_byte();
...
void s3m_t::channel_pan_t::_read() {
    ...
    m_has_custom_pan = m__io->read_bits_int(1);
    m_reserved2 = m__io->read_bits_int(1);
    m_pan = m__io->read_bits_int(4);
}
...
void s3m_t::pattern_cell_t::_read() {
    m_has_fx = m__io->read_bits_int(1);
    m_has_volume = m__io->read_bits_int(1);
    m_has_note_and_instrument = m__io->read_bits_int(1);
    m_channel_num = m__io->read_bits_int(5);
    m__io->align_to_byte();
    n_note = true;
...
void s3m_t::channel_t::_read() {
    m_is_disabled = m__io->read_bits_int(1);
    m_ch_type = m__io->read_bits_int(7);
}
...
void s3m_t::instrument_t::_read() {
    ...
    m_magic = m__io->ensure_fixed_contents(std::string("\x53\x43\x52\x53", 4));
}
...
void s3m_t::instrument_t::adlib_t::_read() {
    m_reserved1 = m__io->ensure_fixed_contents(std::string("\x00\x00\x00", 3));
