...
sub _read {
    ...
    $self->{chunks} = [];
    {
        my $_it;
        do {
            $_it = Dcmp0::Chunk->new($self->{_io}, $self, $self->{_root});
...
sub _read {
    ...
    if ($_on == $Dcmp0::Chunk::TAG_KIND_BACKREFERENCE) {
    ...
    elsif ($_on == $Dcmp0::Chunk::TAG_KIND_EXTENDED) {
...
sub _read {
    ...
    for (my $i = 0; $i < $n_deltas; $i++) {
        push @{$self->{deltas}}, $self->{_io}->read_s1();
    }
}
...
sub _read {
    ...
    for (my $i = 0; $i < $n_deltas_raw; $i++) {
        push @{$self->{deltas_raw}}, DcmpVariableLengthInteger->new($self->{_io});
    }
}
...
sub _read {
    ...
    for (my $i = 0; $i < $n_addresses_raw; $i++) {
        push @{$self->{addresses_raw}}, DcmpVariableLengthInteger->new($self->{_io});
    }
}
