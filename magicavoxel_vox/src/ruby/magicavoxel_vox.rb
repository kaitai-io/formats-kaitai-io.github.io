    ...
    def _read
      ...
      if num_bytes_of_chunk_content != 0
        ...
        when :chunk_type_matt
          _io_chunk_content = @_io.substream(num_bytes_of_chunk_content)
          @chunk_content = Matt.new(_io_chunk_content, self, @_root)
        when :chunk_type_pack
          _io_chunk_content = @_io.substream(num_bytes_of_chunk_content)
          @chunk_content = Pack.new(_io_chunk_content, self, @_root)
        when :chunk_type_rgba
          _io_chunk_content = @_io.substream(num_bytes_of_chunk_content)
          @chunk_content = Rgba.new(_io_chunk_content, self, @_root)
        when :chunk_type_size
          _io_chunk_content = @_io.substream(num_bytes_of_chunk_content)
          @chunk_content = Size.new(_io_chunk_content, self, @_root)
        when :chunk_type_xyzi
          _io_chunk_content = @_io.substream(num_bytes_of_chunk_content)
          @chunk_content = Xyzi.new(_io_chunk_content, self, @_root)
        else
