...
public class Websocket extends KaitaiStruct {
    ...
    public static class Dataframe extends KaitaiStruct {
        ...
        private void _read() {
            ...
            if (_root.dataframes().get((int) 0).opcode() != Websocket.Opcode.TEXT) {
                this.payloadBytes = this._io.readBytes(lenPayload());
            }
            if (_root.dataframes().get((int) 0).opcode() == Websocket.Opcode.TEXT) {
                this.payloadText = new String(this._io.readBytes(lenPayload()), Charset.forName("UTF-8"));
            }
        }
    ...
    public ArrayList<Dataframe> dataframes() { return dataframes; }
    public Websocket _root() { return _root; }
