...
func (this *FalloutDat) Read(io *kaitai.Stream, parent interface{}, root *FalloutDat) (err error) {
	...
	this.FolderNames = make([]*FalloutDat_Pstr, this.FolderCount)
	for i := range this.FolderNames {
		tmp5 := NewFalloutDat_Pstr()
		err = tmp5.Read(this._io, this, this._root)
		if err != nil {
			return err
		}
		this.FolderNames[i] = tmp5
	}
	this.Folders = make([]*FalloutDat_Folder, this.FolderCount)
	for i := range this.Folders {
...
func (this *FalloutDat_Folder) Read(io *kaitai.Stream, parent *FalloutDat, root *FalloutDat) (err error) {
	...
	this.Files = make([]*FalloutDat_File, this.FileCount)
