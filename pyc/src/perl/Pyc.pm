...
sub _read {
    ...
    $self->{body} = Pyc::PyObject->new($self->{_io}, $self, $self->{_root});
}
...
sub _read {
    ...
    $self->{code} = Pyc::Assembly->new($self->{_io}, $self, $self->{_root});
    $self->{consts} = Pyc::PyObject->new($self->{_io}, $self, $self->{_root});
    $self->{names} = Pyc::PyObject->new($self->{_io}, $self, $self->{_root});
    $self->{var_names} = Pyc::PyObject->new($self->{_io}, $self, $self->{_root});
    $self->{free_vars} = Pyc::PyObject->new($self->{_io}, $self, $self->{_root});
    $self->{cell_vars} = Pyc::PyObject->new($self->{_io}, $self, $self->{_root});
    $self->{filename} = Pyc::PyObject->new($self->{_io}, $self, $self->{_root});
    $self->{name} = Pyc::PyObject->new($self->{_io}, $self, $self->{_root});
    $self->{first_line_no} = $self->{_io}->read_u4le();
    $self->{lnotab} = Pyc::PyObject->new($self->{_io}, $self, $self->{_root});
}
...
sub _read {
    ...
    $self->{string_magic} = $self->{_io}->ensure_fixed_contents(pack('C*', (115)));
    $self->{length} = $self->{_io}->read_u4le();
    $self->{_raw_items} = $self->{_io}->read_bytes($self->length());
    my $io__raw_items = IO::KaitaiStruct::Stream->new($self->{_raw_items});
    $self->{items} = Pyc::OpArgs->new($io__raw_items, $self, $self->{_root});
}
...
sub _read {
    ...
    if ($_on == $OBJECT_TYPE_NONE) {
        $self->{value} = Pyc::PyObject::PyNone->new($self->{_io}, $self, $self->{_root});
    }
    elsif ($_on == $OBJECT_TYPE_CODE_OBJECT) {
        $self->{value} = Pyc::CodeObject->new($self->{_io}, $self, $self->{_root});
    }
    elsif ($_on == $OBJECT_TYPE_INT) {
        $self->{value} = $self->{_io}->read_u4le();
    }
    elsif ($_on == $OBJECT_TYPE_STRING_REF) {
        $self->{value} = Pyc::PyObject::StringRef->new($self->{_io}, $self, $self->{_root});
    }
    elsif ($_on == $OBJECT_TYPE_STRING) {
        $self->{value} = Pyc::PyObject::PyString->new($self->{_io}, $self, $self->{_root});
    }
    elsif ($_on == $OBJECT_TYPE_PY_FALSE) {
        $self->{value} = Pyc::PyObject::PyFalse->new($self->{_io}, $self, $self->{_root});
    }
    elsif ($_on == $OBJECT_TYPE_INTERNED) {
        $self->{value} = Pyc::PyObject::InternedString->new($self->{_io}, $self, $self->{_root});
    }
    elsif ($_on == $OBJECT_TYPE_TUPLE) {
        $self->{value} = Pyc::PyObject::Tuple->new($self->{_io}, $self, $self->{_root});
    }
    elsif ($_on == $OBJECT_TYPE_PY_TRUE) {
        $self->{value} = Pyc::PyObject::PyTrue->new($self->{_io}, $self, $self->{_root});
    }
}
...
########################################################################
package Pyc::PyObject::StringRef;
...
sub _read {
    ...
    for (my $i = 0; $i < $n_items; $i++) {
        $self->{items}[$i] = Pyc::PyObject->new($self->{_io}, $self, $self->{_root});
    }
}
...
########################################################################
package Pyc::PyObject::UnicodeString;
...
########################################################################
package Pyc::PyObject::InternedString;
...
sub _read {
    ...
    while (!$self->{_io}->is_eof()) {
        push @{$self->{items}}, Pyc::OpArg->new($self->{_io}, $self, $self->{_root});
    }
}
