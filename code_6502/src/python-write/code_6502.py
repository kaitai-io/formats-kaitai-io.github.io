...
class Code6502(ReadWriteKaitaiStruct):
    ...
    def _write__seq(self, io=None):
        super(Code6502, self)._write__seq(io)
        for i in range(len(self.operations)):
            pass
            if self._io.is_eof():
                raise kaitaistruct.ConsistencyError(u"operations", 0, self._io.size() - self._io.pos())
            self.operations[i]._write__seq(self._io)
    ...
    def _check(self):
        for i in range(len(self.operations)):
            pass
            if self.operations[i]._root != self._root:
                raise kaitaistruct.ConsistencyError(u"operations", self._root, self.operations[i]._root)
            if self.operations[i]._parent != self:
                raise kaitaistruct.ConsistencyError(u"operations", self, self.operations[i]._parent)
    ...
    class Operation(ReadWriteKaitaiStruct):
        ...
        def _write__seq(self, io=None):
            super(Code6502.Operation, self)._write__seq(io)
            self._io.write_u1(int(self.code))
            _on = self.code
            if _on == Code6502.Opcode.adc_abs:
                pass
                self._io.write_u2le(self.args)
            elif _on == Code6502.Opcode.adc_abs_x:
                pass
                self._io.write_u2le(self.args)
            elif _on == Code6502.Opcode.adc_abs_y:
