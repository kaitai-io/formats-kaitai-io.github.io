...
sub _read {
    ...
    $self->{file_time} = Rar::DosTime->new($self->{_io}, $self, $self->{_root});
...
sub year {
    ...
    $self->{year} = ((($self->date() & 65024) >> 9) + 1980);
    return $self->{year};
}

sub minutes {
    my ($self) = @_;
    return $self->{minutes} if ($self->{minutes});
    $self->{minutes} = (($self->time() & 2016) >> 5);
    return $self->{minutes};
}

sub day {
    my ($self) = @_;
    return $self->{day} if ($self->{day});
    $self->{day} = ($self->date() & 31);
    return $self->{day};
}
