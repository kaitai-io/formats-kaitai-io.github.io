  ...
  class Table < Kaitai::Struct::Struct
    ...
    class GlyphNames < Kaitai::Struct::Struct
      ...
      def _read
        ...
        _io_strings = @_io.substream(len_strings)
        @strings = BytesWithIo.new(_io_strings)
        self
      end
    ...
    def body
      ...
      when :types_bdf_encodings
        ...
        @body = BdfEncodings.new(_io_body, self, @_root)
      when :types_bitmaps
        _io_body = @_io.substream(len_body)
        @body = Bitmaps.new(_io_body, self, @_root)
      when :types_glyph_names
        _io_body = @_io.substream(len_body)
        @body = GlyphNames.new(_io_body, self, @_root)
      when :types_properties
        _io_body = @_io.substream(len_body)
        @body = Properties.new(_io_body, self, @_root)
      when :types_swidths
        _io_body = @_io.substream(len_body)
        @body = Swidths.new(_io_body, self, @_root)
      else
