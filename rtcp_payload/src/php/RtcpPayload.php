...
class PsfbAfbRembPacket extends \Kaitai\Struct\Struct {
    ...
    private function _read() {
        ...
        $this->_m_brExp = $this->_io->readBitsInt(6);
        $this->_m_brMantissa = $this->_io->readBitsInt(18);
...
class RtcpPacket extends \Kaitai\Struct\Struct {
    ...
    private function _read() {
        ...
        $this->_m_subtype = $this->_io->readBitsInt(5);
        $this->_io->alignToByte();
...
class PacketStatusChunk extends \Kaitai\Struct\Struct {
    ...
    private function _read() {
        $this->_m_t = $this->_io->readBitsInt(1) != 0;
        if (intval($this->t()) == 0) {
            $this->_m_s2 = $this->_io->readBitsInt(2);
        }
        if (intval($this->t()) == 1) {
            $this->_m_s1 = $this->_io->readBitsInt(1) != 0;
        }
        if (intval($this->t()) == 0) {
            $this->_m_rle = $this->_io->readBitsInt(13);
        }
        if (intval($this->t()) == 1) {
            $this->_m_symbolList = $this->_io->readBitsInt(14);
        }
    }
    protected $_m_s;
    public function s() {
