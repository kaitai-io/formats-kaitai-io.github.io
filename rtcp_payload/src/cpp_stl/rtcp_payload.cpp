...
void rtcp_payload_t::psfb_afb_remb_packet_t::_read() {
    ...
    m_br_exp = m__io->read_bits_int(6);
    m_br_mantissa = m__io->read_bits_int(18);
    m__io->align_to_byte();
...
void rtcp_payload_t::rtpfb_packet_t::_read() {
    ...
    n_fci_block = true;
    switch (fmt()) {
    case RTPFB_SUBTYPE_TRANSPORT_FEEDBACK: {
        n_fci_block = false;
        m__raw_fci_block = m__io->read_bytes_full();
        m__io__raw_fci_block = new kaitai::kstream(m__raw_fci_block);
...
void rtcp_payload_t::packet_status_chunk_t::_read() {
    m_t = m__io->read_bits_int(1);
    n_s2 = true;
    if (((t()) ? 1 : 0) == 0) {
        n_s2 = false;
        m_s2 = m__io->read_bits_int(2);
    }
    n_s1 = true;
    if (((t()) ? 1 : 0) == 1) {
        n_s1 = false;
        m_s1 = m__io->read_bits_int(1);
    }
    n_rle = true;
    if (((t()) ? 1 : 0) == 0) {
        n_rle = false;
        m_rle = m__io->read_bits_int(13);
    }
    n_symbol_list = true;
    if (((t()) ? 1 : 0) == 1) {
        n_symbol_list = false;
        m_symbol_list = m__io->read_bits_int(14);
    }
}
