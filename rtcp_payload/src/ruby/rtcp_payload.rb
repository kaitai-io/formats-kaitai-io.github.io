  ...
  class RtcpPacket < Kaitai::Struct::Struct
    ...
    def _read
      ...
      when :payload_type_psfb
        _io_body = @_io.substream(4 * length)
        @body = PsfbPacket.new(_io_body, self, @_root)
      when :payload_type_rr
        _io_body = @_io.substream(4 * length)
        @body = RrPacket.new(_io_body, self, @_root)
      when :payload_type_rtpfb
        _io_body = @_io.substream(4 * length)
        @body = RtpfbPacket.new(_io_body, self, @_root)
      when :payload_type_sdes
        _io_body = @_io.substream(4 * length)
        @body = SdesPacket.new(_io_body, self, @_root)
      when :payload_type_sr
        _io_body = @_io.substream(4 * length)
        @body = SrPacket.new(_io_body, self, @_root)
      else
