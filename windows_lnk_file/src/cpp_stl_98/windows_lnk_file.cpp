...
std::set<windows_lnk_file_t::drive_types_t> windows_lnk_file_t::_build_values_drive_types_t() {
    std::set<windows_lnk_file_t::drive_types_t> _t;
    _t.insert(windows_lnk_file_t::DRIVE_TYPES_UNKNOWN);
    _t.insert(windows_lnk_file_t::DRIVE_TYPES_NO_ROOT_DIR);
    _t.insert(windows_lnk_file_t::DRIVE_TYPES_REMOVABLE);
    _t.insert(windows_lnk_file_t::DRIVE_TYPES_FIXED);
    _t.insert(windows_lnk_file_t::DRIVE_TYPES_REMOTE);
    _t.insert(windows_lnk_file_t::DRIVE_TYPES_CDROM);
    _t.insert(windows_lnk_file_t::DRIVE_TYPES_RAMDISK);
    return _t;
}
const std::set<windows_lnk_file_t::drive_types_t> windows_lnk_file_t::_values_drive_types_t = windows_lnk_file_t::_build_values_drive_types_t();
bool windows_lnk_file_t::_is_defined_drive_types_t(windows_lnk_file_t::drive_types_t v) {
    return windows_lnk_file_t::_values_drive_types_t.find(v) != windows_lnk_file_t::_values_drive_types_t.end();
}
std::set<windows_lnk_file_t::window_state_t> windows_lnk_file_t::_build_values_window_state_t() {
    std::set<windows_lnk_file_t::window_state_t> _t;
    _t.insert(windows_lnk_file_t::WINDOW_STATE_NORMAL);
    _t.insert(windows_lnk_file_t::WINDOW_STATE_MAXIMIZED);
    _t.insert(windows_lnk_file_t::WINDOW_STATE_MIN_NO_ACTIVE);
    return _t;
}
const std::set<windows_lnk_file_t::window_state_t> windows_lnk_file_t::_values_window_state_t = windows_lnk_file_t::_build_values_window_state_t();
bool windows_lnk_file_t::_is_defined_window_state_t(windows_lnk_file_t::window_state_t v) {
    return windows_lnk_file_t::_values_window_state_t.find(v) != windows_lnk_file_t::_values_window_state_t.end();
}
...
void windows_lnk_file_t::file_header_t::_read() {
    ...
    if (!(m_len_header == std::string("\x4C\x00\x00\x00", 4))) {
        throw kaitai::validation_not_equal_error<std::string>(std::string("\x4C\x00\x00\x00", 4), m_len_header, m__io, std::string("/types/file_header/seq/0"));
    }
    m_link_clsid = m__io->read_bytes(16);
    if (!(m_link_clsid == std::string("\x01\x14\x02\x00\x00\x00\x00\x00\xC0\x00\x00\x00\x00\x00\x00\x46", 16))) {
        throw kaitai::validation_not_equal_error<std::string>(std::string("\x01\x14\x02\x00\x00\x00\x00\x00\xC0\x00\x00\x00\x00\x00\x00\x46", 16), m_link_clsid, m__io, std::string("/types/file_header/seq/1"));
    }
    ...
    if (!(m_reserved == std::string("\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 10))) {
        throw kaitai::validation_not_equal_error<std::string>(std::string("\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00", 10), m_reserved, m__io, std::string("/types/file_header/seq/11"));
    }
}
...
std::string windows_lnk_file_t::link_info_t::volume_id_body_t::volume_label_ansi() {
    ...
    if (!(is_unicode())) {
        ...
        m_volume_label_ansi = kaitai::kstream::bytes_to_str(m__io->read_bytes_term(0, false, true, true), "IBM437");
        m__io->seek(_pos);
    }
    return m_volume_label_ansi;
}
...
void windows_lnk_file_t::string_data_t::_read() {
    ...
    m_str = kaitai::kstream::bytes_to_str(m__io->read_bytes(chars_str() * 2), "UTF-16LE");
}
