...
sub ifd0 {
    ...
    $self->{ifd0} = ExifLe::Ifd->new($self->{_io}, $self, $self->{_root});
    $self->{_io}->seek($_pos);
    return $self->{ifd0};
}
...
sub _read {
    ...
    for (my $i = 0; $i < $n_fields; $i++) {
        $self->{fields}[$i] = ExifLe::IfdField->new($self->{_io}, $self, $self->{_root});
    }
...
sub next_ifd {
    ...
    if ($self->next_ifd_ofs() != 0) {
        ...
        $self->{next_ifd} = ExifLe::Ifd->new($self->{_io}, $self, $self->{_root});
        $self->{_io}->seek($_pos);
    }
...
sub type_byte_length {
    ...
    $self->{type_byte_length} = ($self->field_type() == $ExifLe::IfdField::FIELD_TYPE_ENUM_WORD ? 2 : ($self->field_type() == $ExifLe::IfdField::FIELD_TYPE_ENUM_DWORD ? 4 : 1));
