...
class Dcmp2 < Kaitai::Struct::Struct
  def initialize(_io, _parent = nil, _root = nil, len_decompressed, header_parameters_with_io)
  ...
  def _read
    ...
    when true
      _io_data = @_io.substream((_io.size - _io.pos) - (is_len_decompressed_odd ? 1 : 0))
      @data = TaggedData.new(_io_data, self, @_root)
    else
      _io_data = @_io.substream((_io.size - _io.pos) - (is_len_decompressed_odd ? 1 : 0))
      @data = UntaggedData.new(_io_data, self, @_root)
    end
