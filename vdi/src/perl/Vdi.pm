...
sub blocks_map {
    ...
    $self->{_raw_blocks_map} = $self->{_io}->read_bytes($self->header()->bmap_size());
...
sub bmap_size {
    my ($self) = @_;
    return $self->{bmap_size} if ($self->{bmap_size});
    $self->{bmap_size} = (int(((($self->header_main()->blocks_in_image() * 4) + $self->header_main()->geometry()->sector_size()) - 1) / $self->header_main()->geometry()->sector_size()) * $self->header_main()->geometry()->sector_size());
    return $self->{bmap_size};
}
...
sub bmap_offset {
    my ($self) = @_;
    return $self->{bmap_offset} if ($self->{bmap_offset});
    $self->{bmap_offset} = $self->header_main()->offset_bmap();
    return $self->{bmap_offset};
}
