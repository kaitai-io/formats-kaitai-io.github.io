...
namespace Kaitai
{
    ...
    public partial class Dtb : KaitaiStruct
    {
        ...
        private void _read()
        {
            ...
            if (!((KaitaiStream.ByteArrayCompare(_magic, new byte[] { 208, 13, 254, 237 }) == 0)))
            {
                throw new ValidationNotEqualError(new byte[] { 208, 13, 254, 237 }, _magic, m_io, "/seq/0");
            }
            ...
            if (!(_minCompatibleVersion <= Version))
            {
                throw new ValidationGreaterThanError(Version, _minCompatibleVersion, m_io, "/seq/6");
            }
