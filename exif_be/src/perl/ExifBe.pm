...
sub ifd0 {
    ...
    $self->{ifd0} = ExifBe::Ifd->new($self->{_io}, $self, $self->{_root});
    $self->{_io}->seek($_pos);
    return $self->{ifd0};
}
...
sub _read {
    ...
    for (my $i = 0; $i < $n_fields; $i++) {
        $self->{fields}[$i] = ExifBe::IfdField->new($self->{_io}, $self, $self->{_root});
    }
...
sub next_ifd {
    ...
    if ($self->next_ifd_ofs() != 0) {
        ...
        $self->{next_ifd} = ExifBe::Ifd->new($self->{_io}, $self, $self->{_root});
        $self->{_io}->seek($_pos);
    }
