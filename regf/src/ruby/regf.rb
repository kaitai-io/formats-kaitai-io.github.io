...
class Regf < Kaitai::Struct::Struct
  ...
  def _read
    ...
    i = 0
    while not @_io.eof?
      _io_hive_bins = @_io.substream(4096)
      @hive_bins << HiveBin.new(_io_hive_bins, self, @_root)
      i += 1
    end
    self
  end
  ...
  class HiveBinCell < Kaitai::Struct::Struct
    ...
    def _read
      ...
      @identifier = (@_io.read_bytes(2)).force_encoding("ASCII").encode('UTF-8')
      case identifier
      when "lf"
        _io_data = @_io.substream((cell_size - 2) - 4)
        @data = SubKeyListLhLf.new(_io_data, self, @_root)
      when "lh"
        _io_data = @_io.substream((cell_size - 2) - 4)
        @data = SubKeyListLhLf.new(_io_data, self, @_root)
      when "li"
        _io_data = @_io.substream((cell_size - 2) - 4)
        @data = SubKeyListLi.new(_io_data, self, @_root)
      when "nk"
        _io_data = @_io.substream((cell_size - 2) - 4)
        @data = NamedKey.new(_io_data, self, @_root)
      when "ri"
        _io_data = @_io.substream((cell_size - 2) - 4)
        @data = SubKeyListRi.new(_io_data, self, @_root)
      when "sk"
        _io_data = @_io.substream((cell_size - 2) - 4)
        @data = SubKeyListSk.new(_io_data, self, @_root)
      when "vk"
        _io_data = @_io.substream((cell_size - 2) - 4)
        @data = SubKeyListVk.new(_io_data, self, @_root)
      else
    ...
    class NamedKey < Kaitai::Struct::Struct
      ...
      def _read
        ...
        @unknown_string = (@_io.read_bytes(unknown_string_size)).force_encoding("ASCII").encode('UTF-8')
        self
      end
      attr_reader :flags
    ...
    class SubKeyListVk < Kaitai::Struct::Struct
      ...
      def _read
        ...
        if flags == :vk_flags_value_comp_name
          @value_name = (@_io.read_bytes(value_name_size)).force_encoding("ASCII").encode('UTF-8')
        end
        self
      end
