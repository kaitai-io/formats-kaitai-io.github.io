...
class Cramfs(KaitaiStruct):
    ...
    class SuperBlockStruct(KaitaiStruct):
        ...
        def _read(self):
            self.magic = self._io.ensure_fixed_contents(struct.pack('4b', 69, 61, -51, 40))
            self.size = self._io.read_u4le()
            self.flags = self._io.read_u4le()
            self.future = self._io.read_u4le()
            self.signature = self._io.ensure_fixed_contents(struct.pack('16b', 67, 111, 109, 112, 114, 101, 115, 115, 101, 100, 32, 82, 79, 77, 70, 83))
