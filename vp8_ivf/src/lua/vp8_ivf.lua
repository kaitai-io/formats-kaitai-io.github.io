...
function Vp8Ivf:_init(io, parent, root)
...
function Vp8Ivf:_read()
  ...
  for i = 0, self.num_frames - 1 do
    self.image_data[i + 1] = Vp8Ivf.Blocks(self._io, self, self._root)
  end
end
...
Vp8Ivf.Blocks = class.class(KaitaiStruct)

function Vp8Ivf.Blocks:_init(io, parent, root)
  KaitaiStruct._init(self, io)
  self._parent = parent
  self._root = root or self
  self:_read()
...
function Vp8Ivf.Blocks:_read()
  self.entries = Vp8Ivf.Block(self._io, self, self._root)
end


Vp8Ivf.Block = class.class(KaitaiStruct)

function Vp8Ivf.Block:_init(io, parent, root)
  KaitaiStruct._init(self, io)
  self._parent = parent
  self._root = root or self
  self:_read()
