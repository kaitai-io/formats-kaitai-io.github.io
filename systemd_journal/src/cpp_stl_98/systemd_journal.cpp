// This is a generated file! Please edit source .ksy file and use kaitai-struct-compiler to rebuild

#include "systemd_journal.h"
#include "kaitai/exceptions.h"
std::set<systemd_journal_t::state_t> systemd_journal_t::_build_values_state_t() {
    std::set<systemd_journal_t::state_t> _t;
    _t.insert(systemd_journal_t::STATE_OFFLINE);
    _t.insert(systemd_journal_t::STATE_ONLINE);
    _t.insert(systemd_journal_t::STATE_ARCHIVED);
    return _t;
}
const std::set<systemd_journal_t::state_t> systemd_journal_t::_values_state_t = systemd_journal_t::_build_values_state_t();
bool systemd_journal_t::_is_defined_state_t(systemd_journal_t::state_t v) {
    return systemd_journal_t::_values_state_t.find(v) != systemd_journal_t::_values_state_t.end();
}

systemd_journal_t::systemd_journal_t(kaitai::kstream* p__io, kaitai::kstruct* p__parent, systemd_journal_t* p__root) : kaitai::kstruct(p__io) {
    m__parent = p__parent;
    m__root = p__root ? p__root : this;
    m_header = 0;
    m__io__raw_header = 0;
    m_objects = 0;
    f_data_hash_table = false;
    f_field_hash_table = false;
    f_len_header = false;

    try {
        _read();
    } catch(...) {
        _clean_up();
        throw;
    }
}

void systemd_journal_t::_read() {
    m__raw_header = m__io->read_bytes(len_header());
    m__io__raw_header = new kaitai::kstream(m__raw_header);
    m_header = new header_t(m__io__raw_header, this, m__root);
    m_objects = new std::vector<journal_object_t*>();
    const int l_objects = header()->num_objects();
    for (int i = 0; i < l_objects; i++) {
        m_objects->push_back(new journal_object_t(m__io, this, m__root));
    }
}

systemd_journal_t::~systemd_journal_t() {
    _clean_up();
}

void systemd_journal_t::_clean_up() {
    if (m__io__raw_header) {
        delete m__io__raw_header; m__io__raw_header = 0;
    }
    if (m_header) {
        delete m_header; m_header = 0;
    }
    if (m_objects) {
        for (std::vector<journal_object_t*>::iterator it = m_objects->begin(); it != m_objects->end(); ++it) {
            delete *it;
        }
        delete m_objects; m_objects = 0;
    }
    if (f_data_hash_table) {
    }
    if (f_field_hash_table) {
    }
    if (f_len_header) {
    }
}

systemd_journal_t::data_object_t::data_object_t(kaitai::kstream* p__io, systemd_journal_t::journal_object_t* p__parent, systemd_journal_t* p__root) : kaitai::kstruct(p__io) {
    m__parent = p__parent;
    m__root = p__root;
    m_entry = 0;
    m_entry_array = 0;
    m_head_field = 0;
    m_next_hash = 0;
    f_entry = false;
    f_entry_array = false;
    f_head_field = false;
    f_next_hash = false;

    try {
        _read();
    } catch(...) {
        _clean_up();
        throw;
    }
}

void systemd_journal_t::data_object_t::_read() {
    m_hash = m__io->read_u8le();
    m_ofs_next_hash = m__io->read_u8le();
    m_ofs_head_field = m__io->read_u8le();
    m_ofs_entry = m__io->read_u8le();
    m_ofs_entry_array = m__io->read_u8le();
    m_num_entries = m__io->read_u8le();
    m_payload = m__io->read_bytes_full();
}

systemd_journal_t::data_object_t::~data_object_t() {
    _clean_up();
}

void systemd_journal_t::data_object_t::_clean_up() {
    if (f_entry && !n_entry) {
        if (m_entry) {
            delete m_entry; m_entry = 0;
        }
    }
    if (f_entry_array && !n_entry_array) {
        if (m_entry_array) {
            delete m_entry_array; m_entry_array = 0;
        }
    }
    if (f_head_field && !n_head_field) {
        if (m_head_field) {
            delete m_head_field; m_head_field = 0;
        }
    }
    if (f_next_hash && !n_next_hash) {
        if (m_next_hash) {
            delete m_next_hash; m_next_hash = 0;
        }
    }
}

systemd_journal_t::journal_object_t* systemd_journal_t::data_object_t::entry() {
    if (f_entry)
        return m_entry;
    f_entry = true;
    n_entry = true;
    if (ofs_entry() != 0) {
        n_entry = false;
        kaitai::kstream *io = _root()->_io();
        std::streampos _pos = io->pos();
        io->seek(ofs_entry());
        m_entry = new journal_object_t(io, this, m__root);
        io->seek(_pos);
    }
    return m_entry;
}

systemd_journal_t::journal_object_t* systemd_journal_t::data_object_t::entry_array() {
    if (f_entry_array)
        return m_entry_array;
    f_entry_array = true;
    n_entry_array = true;
    if (ofs_entry_array() != 0) {
        n_entry_array = false;
        kaitai::kstream *io = _root()->_io();
        std::streampos _pos = io->pos();
        io->seek(ofs_entry_array());
        m_entry_array = new journal_object_t(io, this, m__root);
        io->seek(_pos);
    }
    return m_entry_array;
}

systemd_journal_t::journal_object_t* systemd_journal_t::data_object_t::head_field() {
    if (f_head_field)
        return m_head_field;
    f_head_field = true;
    n_head_field = true;
    if (ofs_head_field() != 0) {
        n_head_field = false;
        kaitai::kstream *io = _root()->_io();
        std::streampos _pos = io->pos();
        io->seek(ofs_head_field());
        m_head_field = new journal_object_t(io, this, m__root);
        io->seek(_pos);
    }
    return m_head_field;
}

systemd_journal_t::journal_object_t* systemd_journal_t::data_object_t::next_hash() {
    if (f_next_hash)
        return m_next_hash;
    f_next_hash = true;
    n_next_hash = true;
    if (ofs_next_hash() != 0) {
        n_next_hash = false;
        kaitai::kstream *io = _root()->_io();
        std::streampos _pos = io->pos();
        io->seek(ofs_next_hash());
        m_next_hash = new journal_object_t(io, this, m__root);
        io->seek(_pos);
    }
    return m_next_hash;
}

systemd_journal_t::header_t::header_t(kaitai::kstream* p__io, systemd_journal_t* p__parent, systemd_journal_t* p__root) : kaitai::kstruct(p__io) {
    m__parent = p__parent;
    m__root = p__root;

    try {
        _read();
    } catch(...) {
        _clean_up();
        throw;
    }
}

void systemd_journal_t::header_t::_read() {
    m_signature = m__io->read_bytes(8);
    if (!(m_signature == std::string("\x4C\x50\x4B\x53\x48\x48\x52\x48", 8))) {
        throw kaitai::validation_not_equal_error<std::string>(std::string("\x4C\x50\x4B\x53\x48\x48\x52\x48", 8), m_signature, m__io, std::string("/types/header/seq/0"));
    }
    m_compatible_flags = m__io->read_u4le();
    m_incompatible_flags = m__io->read_u4le();
    m_state = static_cast<systemd_journal_t::state_t>(m__io->read_u1());
    m_reserved = m__io->read_bytes(7);
    m_file_id = m__io->read_bytes(16);
    m_machine_id = m__io->read_bytes(16);
    m_boot_id = m__io->read_bytes(16);
    m_seqnum_id = m__io->read_bytes(16);
    m_len_header = m__io->read_u8le();
    m_len_arena = m__io->read_u8le();
    m_ofs_data_hash_table = m__io->read_u8le();
    m_len_data_hash_table = m__io->read_u8le();
    m_ofs_field_hash_table = m__io->read_u8le();
    m_len_field_hash_table = m__io->read_u8le();
    m_ofs_tail_object = m__io->read_u8le();
    m_num_objects = m__io->read_u8le();
    m_num_entries = m__io->read_u8le();
    m_tail_entry_seqnum = m__io->read_u8le();
    m_head_entry_seqnum = m__io->read_u8le();
    m_ofs_entry_array = m__io->read_u8le();
    m_head_entry_realtime = m__io->read_u8le();
    m_tail_entry_realtime = m__io->read_u8le();
    m_tail_entry_monotonic = m__io->read_u8le();
    n_num_data = true;
    if (!(_io()->is_eof())) {
        n_num_data = false;
        m_num_data = m__io->read_u8le();
    }
    n_num_fields = true;
    if (!(_io()->is_eof())) {
        n_num_fields = false;
        m_num_fields = m__io->read_u8le();
    }
    n_num_tags = true;
    if (!(_io()->is_eof())) {
        n_num_tags = false;
        m_num_tags = m__io->read_u8le();
    }
    n_num_entry_arrays = true;
    if (!(_io()->is_eof())) {
        n_num_entry_arrays = false;
        m_num_entry_arrays = m__io->read_u8le();
    }
}

systemd_journal_t::header_t::~header_t() {
    _clean_up();
}

void systemd_journal_t::header_t::_clean_up() {
    if (!n_num_data) {
    }
    if (!n_num_fields) {
    }
    if (!n_num_tags) {
    }
    if (!n_num_entry_arrays) {
    }
}
std::set<systemd_journal_t::journal_object_t::object_types_t> systemd_journal_t::journal_object_t::_build_values_object_types_t() {
    std::set<systemd_journal_t::journal_object_t::object_types_t> _t;
    _t.insert(systemd_journal_t::journal_object_t::OBJECT_TYPES_UNUSED);
    _t.insert(systemd_journal_t::journal_object_t::OBJECT_TYPES_DATA);
    _t.insert(systemd_journal_t::journal_object_t::OBJECT_TYPES_FIELD);
    _t.insert(systemd_journal_t::journal_object_t::OBJECT_TYPES_ENTRY);
    _t.insert(systemd_journal_t::journal_object_t::OBJECT_TYPES_DATA_HASH_TABLE);
    _t.insert(systemd_journal_t::journal_object_t::OBJECT_TYPES_FIELD_HASH_TABLE);
    _t.insert(systemd_journal_t::journal_object_t::OBJECT_TYPES_ENTRY_ARRAY);
    _t.insert(systemd_journal_t::journal_object_t::OBJECT_TYPES_TAG);
    return _t;
}
const std::set<systemd_journal_t::journal_object_t::object_types_t> systemd_journal_t::journal_object_t::_values_object_types_t = systemd_journal_t::journal_object_t::_build_values_object_types_t();
bool systemd_journal_t::journal_object_t::_is_defined_object_types_t(systemd_journal_t::journal_object_t::object_types_t v) {
    return systemd_journal_t::journal_object_t::_values_object_types_t.find(v) != systemd_journal_t::journal_object_t::_values_object_types_t.end();
}

systemd_journal_t::journal_object_t::journal_object_t(kaitai::kstream* p__io, kaitai::kstruct* p__parent, systemd_journal_t* p__root) : kaitai::kstruct(p__io) {
    m__parent = p__parent;
    m__root = p__root;
    m__io__raw_payload = 0;

    try {
        _read();
    } catch(...) {
        _clean_up();
        throw;
    }
}

void systemd_journal_t::journal_object_t::_read() {
    m_padding = m__io->read_bytes(kaitai::kstream::mod(8 - _io()->pos(), 8));
    m_object_type = static_cast<systemd_journal_t::journal_object_t::object_types_t>(m__io->read_u1());
    m_flags = m__io->read_u1();
    m_reserved = m__io->read_bytes(6);
    m_len_object = m__io->read_u8le();
    n_payload = true;
    switch (object_type()) {
    case systemd_journal_t::journal_object_t::OBJECT_TYPES_DATA: {
        n_payload = false;
        m__raw_payload = m__io->read_bytes(len_object() - 16);
        m__io__raw_payload = new kaitai::kstream(m__raw_payload);
        m_payload = new data_object_t(m__io__raw_payload, this, m__root);
        break;
    }
    default: {
        m__raw_payload = m__io->read_bytes(len_object() - 16);
        break;
    }
    }
}

systemd_journal_t::journal_object_t::~journal_object_t() {
    _clean_up();
}

void systemd_journal_t::journal_object_t::_clean_up() {
    if (!n_payload) {
        if (m__io__raw_payload) {
            delete m__io__raw_payload; m__io__raw_payload = 0;
        }
        if (m_payload) {
            delete m_payload; m_payload = 0;
        }
    }
}

std::string systemd_journal_t::data_hash_table() {
    if (f_data_hash_table)
        return m_data_hash_table;
    f_data_hash_table = true;
    std::streampos _pos = m__io->pos();
    m__io->seek(header()->ofs_data_hash_table());
    m_data_hash_table = m__io->read_bytes(header()->len_data_hash_table());
    m__io->seek(_pos);
    return m_data_hash_table;
}

std::string systemd_journal_t::field_hash_table() {
    if (f_field_hash_table)
        return m_field_hash_table;
    f_field_hash_table = true;
    std::streampos _pos = m__io->pos();
    m__io->seek(header()->ofs_field_hash_table());
    m_field_hash_table = m__io->read_bytes(header()->len_field_hash_table());
    m__io->seek(_pos);
    return m_field_hash_table;
}

uint64_t systemd_journal_t::len_header() {
    if (f_len_header)
        return m_len_header;
    f_len_header = true;
    std::streampos _pos = m__io->pos();
    m__io->seek(88);
    m_len_header = m__io->read_u8le();
    m__io->seek(_pos);
    return m_len_header;
}
