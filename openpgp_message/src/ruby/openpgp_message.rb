...
class OpenpgpMessage < Kaitai::Struct::Struct
  ...
  HASH_ALGORITHMS = {
    ...
    4 => :hash_algorithms_reserved_4,
    5 => :hash_algorithms_reserved_5,
    6 => :hash_algorithms_reserved_6,
    7 => :hash_algorithms_reserved_7,
  ...
  SYMMETRIC_KEY_ALGORITHM = {
    ...
    5 => :symmetric_key_algorithm_reserved_5,
    6 => :symmetric_key_algorithm_reserved_6,
  ...
  SUBPACKET_TYPES = {
    0 => :subpacket_types_reserved,
    1 => :subpacket_types_reserved,
    ...
    13 => :subpacket_types_reserved,
    14 => :subpacket_types_reserved,
    15 => :subpacket_types_reserved,
    16 => :subpacket_types_issuer,
    17 => :subpacket_types_reserved,
    18 => :subpacket_types_reserved,
    19 => :subpacket_types_reserved,
  ...
  class PreferredHashAlgorithms < Kaitai::Struct::Struct
    ...
    def _read
      ...
      i = 0
      while not @_io.eof?
        @algorithm << Kaitai::Struct::Stream::resolve_enum(HASH_ALGORITHMS, @_io.read_u1)
        i += 1
      end
      self
    end
  ...
  class PreferredCompressionAlgorithms < Kaitai::Struct::Struct
    ...
    def _read
      ...
      i = 0
      while not @_io.eof?
        @algorithm << Kaitai::Struct::Stream::resolve_enum(COMPRESSION_ALGORITHMS, @_io.read_u1)
        i += 1
      end
      self
    end
  ...
  class SecretKeyPacket < Kaitai::Struct::Struct
    ...
    def _read
      ...
      if string_to_key >= 254
        @symmetric_encryption_algorithm = Kaitai::Struct::Stream::resolve_enum(SYMMETRIC_KEY_ALGORITHM, @_io.read_u1)
      end
  ...
  class KeyServerPreferences < Kaitai::Struct::Struct
    ...
    def _read
      ...
      i = 0
      while not @_io.eof?
        @flag << Kaitai::Struct::Stream::resolve_enum(SERVER_FLAGS, @_io.read_u1)
        i += 1
      end
      self
    end
  ...
  class RevocationKey < Kaitai::Struct::Struct
    ...
    def _read
      @class = @_io.read_u1
      @public_key_algorithm = Kaitai::Struct::Stream::resolve_enum(PUBLIC_KEY_ALGORITHMS, @_io.read_u1)
  ...
  class SignatureTarget < Kaitai::Struct::Struct
    ...
    def _read
      @public_key_algorithm = Kaitai::Struct::Stream::resolve_enum(PUBLIC_KEY_ALGORITHMS, @_io.read_u1)
      @hash_algorithm = Kaitai::Struct::Stream::resolve_enum(HASH_ALGORITHMS, @_io.read_u1)
  ...
  class KeyFlags < Kaitai::Struct::Struct
    ...
    def _read
      ...
      i = 0
      while not @_io.eof?
        @flag << Kaitai::Struct::Stream::resolve_enum(KEY_FLAGS, @_io.read_u1)
        i += 1
      end
      self
    end
  ...
  class Subpacket < Kaitai::Struct::Struct
    ...
    def _read
      ...
      @subpacket_type = Kaitai::Struct::Stream::resolve_enum(SUBPACKET_TYPES, @_io.read_u1)
      case subpacket_type
      when :subpacket_types_exportable_certification
        ...
        @content = ExportableCertification.new(io, self, @_root)
      when :subpacket_types_preferred_hash_algorithms
        @_raw_content = @_io.read_bytes((len.len - 1))
        io = Kaitai::Struct::Stream.new(@_raw_content)
        @content = PreferredHashAlgorithms.new(io, self, @_root)
      when :subpacket_types_embedded_signature
        @_raw_content = @_io.read_bytes((len.len - 1))
        io = Kaitai::Struct::Stream.new(@_raw_content)
        @content = EmbeddedSignature.new(io, self, @_root)
      when :subpacket_types_trust_signature
        @_raw_content = @_io.read_bytes((len.len - 1))
        io = Kaitai::Struct::Stream.new(@_raw_content)
        @content = TrustSignature.new(io, self, @_root)
      when :subpacket_types_reason_for_revocation
        @_raw_content = @_io.read_bytes((len.len - 1))
        io = Kaitai::Struct::Stream.new(@_raw_content)
        @content = ReasonForRevocation.new(io, self, @_root)
      when :subpacket_types_features
        @_raw_content = @_io.read_bytes((len.len - 1))
        io = Kaitai::Struct::Stream.new(@_raw_content)
        @content = Features.new(io, self, @_root)
      when :subpacket_types_signature_expiration_time
        @_raw_content = @_io.read_bytes((len.len - 1))
        io = Kaitai::Struct::Stream.new(@_raw_content)
        @content = SignatureExpirationTime.new(io, self, @_root)
      when :subpacket_types_preferred_compression_algorithms
        @_raw_content = @_io.read_bytes((len.len - 1))
        io = Kaitai::Struct::Stream.new(@_raw_content)
        @content = PreferredCompressionAlgorithms.new(io, self, @_root)
      when :subpacket_types_revocable
        @_raw_content = @_io.read_bytes((len.len - 1))
        io = Kaitai::Struct::Stream.new(@_raw_content)
        @content = Revocable.new(io, self, @_root)
      when :subpacket_types_regular_expression
        @_raw_content = @_io.read_bytes((len.len - 1))
        io = Kaitai::Struct::Stream.new(@_raw_content)
        @content = RegularExpression.new(io, self, @_root)
      when :subpacket_types_notation_data
        @_raw_content = @_io.read_bytes((len.len - 1))
        io = Kaitai::Struct::Stream.new(@_raw_content)
        @content = NotationData.new(io, self, @_root)
      when :subpacket_types_key_expiration_time
        @_raw_content = @_io.read_bytes((len.len - 1))
        io = Kaitai::Struct::Stream.new(@_raw_content)
        @content = KeyExpirationTime.new(io, self, @_root)
      when :subpacket_types_signature_creation_time
        @_raw_content = @_io.read_bytes((len.len - 1))
        io = Kaitai::Struct::Stream.new(@_raw_content)
        @content = SignatureCreationTime.new(io, self, @_root)
      when :subpacket_types_revocation_key
        @_raw_content = @_io.read_bytes((len.len - 1))
        io = Kaitai::Struct::Stream.new(@_raw_content)
        @content = RevocationKey.new(io, self, @_root)
      when :subpacket_types_key_flags
        @_raw_content = @_io.read_bytes((len.len - 1))
        io = Kaitai::Struct::Stream.new(@_raw_content)
        @content = KeyFlags.new(io, self, @_root)
      when :subpacket_types_issuer
        @_raw_content = @_io.read_bytes((len.len - 1))
        io = Kaitai::Struct::Stream.new(@_raw_content)
        @content = Issuer.new(io, self, @_root)
      when :subpacket_types_signature_target
        @_raw_content = @_io.read_bytes((len.len - 1))
        io = Kaitai::Struct::Stream.new(@_raw_content)
        @content = SignatureTarget.new(io, self, @_root)
      when :subpacket_types_key_server_preferences
        @_raw_content = @_io.read_bytes((len.len - 1))
        io = Kaitai::Struct::Stream.new(@_raw_content)
        @content = KeyServerPreferences.new(io, self, @_root)
      when :subpacket_types_signers_user_id
        @_raw_content = @_io.read_bytes((len.len - 1))
        io = Kaitai::Struct::Stream.new(@_raw_content)
        @content = SignersUserId.new(io, self, @_root)
      when :subpacket_types_policy_uri
        @_raw_content = @_io.read_bytes((len.len - 1))
        io = Kaitai::Struct::Stream.new(@_raw_content)
        @content = PolicyUri.new(io, self, @_root)
      when :subpacket_types_preferred_key_server
        @_raw_content = @_io.read_bytes((len.len - 1))
        io = Kaitai::Struct::Stream.new(@_raw_content)
        @content = PreferredKeyServer.new(io, self, @_root)
      when :subpacket_types_primary_user_id
        @_raw_content = @_io.read_bytes((len.len - 1))
        io = Kaitai::Struct::Stream.new(@_raw_content)
        @content = PrimaryUserId.new(io, self, @_root)
      else
  ...
  class OldPacket < Kaitai::Struct::Struct
    ...
    def _read
      ...
      when :packet_tags_public_key_packet
        ...
        @body = PublicKeyPacket.new(io, self, @_root)
      ...
      when :packet_tags_user_id_packet
        @_raw_body = @_io.read_bytes(len)
        io = Kaitai::Struct::Stream.new(@_raw_body)
        @body = UserIdPacket.new(io, self, @_root)
      when :packet_tags_secret_key_packet
        @_raw_body = @_io.read_bytes(len)
        io = Kaitai::Struct::Stream.new(@_raw_body)
        @body = SecretKeyPacket.new(io, self, @_root)
      when :packet_tags_signature_packet
        @_raw_body = @_io.read_bytes(len)
        io = Kaitai::Struct::Stream.new(@_raw_body)
        @body = SignaturePacket.new(io, self, @_root)
  ...
  class SignaturePacket < Kaitai::Struct::Struct
    ...
    def _read
      ...
      io = Kaitai::Struct::Stream.new(@_raw_hashed_subpackets)
      @hashed_subpackets = Subpackets.new(io, self, @_root)
      @len_unhashed_subpacket = @_io.read_u2be
      @_raw_unhashed_subpackets = @_io.read_bytes(len_unhashed_subpacket)
      io = Kaitai::Struct::Stream.new(@_raw_unhashed_subpackets)
      @unhashed_subpackets = Subpackets.new(io, self, @_root)
  ...
  class ReasonForRevocation < Kaitai::Struct::Struct
    ...
    def _read
      @revocation_code = Kaitai::Struct::Stream::resolve_enum(REVOCATION_CODES, @_io.read_u1)
  ...
  class Packet < Kaitai::Struct::Struct
    ...
    def _read
      @one = @_io.read_bits_int(1) != 0
      @new_packet_format = @_io.read_bits_int(1) != 0
      if new_packet_format
        @packet_type_new = Kaitai::Struct::Stream::resolve_enum(PACKET_TAGS, @_io.read_bits_int(6))
      end
      if !(new_packet_format)
        @packet_type_old = Kaitai::Struct::Stream::resolve_enum(PACKET_TAGS, @_io.read_bits_int(4))
      end
      if !(new_packet_format)
        @len_type = @_io.read_bits_int(2)
      end
      @_io.align_to_byte
