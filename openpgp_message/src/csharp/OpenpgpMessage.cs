...
namespace Kaitai
{
    ...
    public partial class OpenpgpMessage : KaitaiStruct
    {
        ...
        public partial class Packet : KaitaiStruct
        {
            ...
            private void _read()
            {
                _one = m_io.ReadBitsInt(1) != 0;
                _newPacketFormat = m_io.ReadBitsInt(1) != 0;
                if (NewPacketFormat) {
                    _packetTypeNew = ((OpenpgpMessage.PacketTags) m_io.ReadBitsInt(6));
                }
                if (!(NewPacketFormat)) {
                    _packetTypeOld = ((OpenpgpMessage.PacketTags) m_io.ReadBitsInt(4));
                }
                if (!(NewPacketFormat)) {
                    _lenType = m_io.ReadBitsInt(2);
                }
                m_io.AlignToByte();
                {
